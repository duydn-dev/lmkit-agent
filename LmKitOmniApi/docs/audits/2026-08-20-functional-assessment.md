...
| 9 | MCP integration | **8,0** | Dùng official MCP C# SDK với Streamable HTTP, discovery/capability negotiation, JSON-RPC `tools/list`/`tools/call`, chuẩn header 2026-07-28 và fallback initialize/session cho server cũ. Có tenant-scoped CRUD/UI, SSRF/DNS/redirect guard, encrypted headers, cache theo tenant, schema nguyên bản và server-qualified invocation. Mọi tool mặc định cần HITL; `readOnlyHint=true` chỉ được tự chạy khi Tenant Admin bật trust policy riêng cho server. Còn thiếu OAuth authorization flow, upstream conformance suite và E2E với MCP server production thật. |
| 10 | Tool RBAC, sandbox, resilience, HITL và audit | **8,4** | Approved action re-check quyền hiện tại; write/unknown action không retry; regression test chứng minh chạy một lần. AI quota và circuit breaker đều dùng Redis atomic; breaker tách tenant/tool, hash key, dừng retry khi mở và chỉ cho một half-open probe giữa nhiều replica. Khi Redis mất kết nối, breaker hạ cấp sang state process-local có khóa thay vì làm hỏng tool call. Còn thiếu policy editor chi tiết hơn ba role tĩnh và chaos/load benchmark production. |
...
| 22 | Test và CI/CD | **8,9** | 102 backend, 16 frontend unit/source, 4 browser contract/axe và 1 browser full-stack test pass. Có wire-level MCP và Redis concurrency/half-open/disconnect regression; backend CI chạy Redis service thật. CI full-stack dựng Nginx/API/PostgreSQL/Redis/Qdrant và kiểm tra auth/session/document/user thật. Release/Docker build, npm/NuGet vulnerability audit và migration drift sạch. Chưa có model-backed E2E, coverage gate, AI golden set, load/chaos/DAST. |
...
- Thay REST MCP adapter riêng bằng official SDK/Streamable HTTP; thêm protocol negotiation, server-qualified tool routing, schema fidelity và fail-safe HITL với trust policy tường minh cho `readOnlyHint`.
...
3. MCP đã dùng transport chuẩn nhưng chưa chạy upstream conformance suite, OAuth flow và E2E với server production thật.
...
- `dotnet test ... -c Release`: **102/102 pass**, gồm HTTP integration cho auth cookie, authorization, memory consent/tenant scope, approval isolation/idempotency, session missing/cross-tenant contract, MCP wire/routing/approval, Redis atomic circuit/concurrent half-open/disconnect fallback, local rate-limit contract, attachment batch quota và cleanup scratch file.
...
- Chạy MCP conformance/OAuth và E2E với server đích trước production; nếu cần public widget, triển khai credential/origin contract riêng thay vì bỏ auth.
